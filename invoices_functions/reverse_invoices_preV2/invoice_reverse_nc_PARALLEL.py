...
def reverse_invoice_meli(): #NOTAS DE CRÉDITO INDIVIDUALES MELI
    ...
    #INDIVIDUALES MELI
    mycursor.execute("""#MERCADO-LIBRE TOTALES
                        #INDIVIDUALES
                        SELECT c.name,
                        ...
                        LEFT JOIN (SELECT a.order_id, max(payment_date_last_modified) 'payment_date_last_modified', SUM(paid_amt) 'paid_amt', SUM(refunded_amt) 'refunded_amt', SUM(shipping_amt) 'shipping_amt'
                                   ...
                                   WHERE refunded_amt > 0
                                     AND b.pack_id = 'None'
                                     AND date(payment_date_last_modified) >= %s
                                     AND date(payment_date_last_modified) <= %s
    ...
    try:
        ...
        for each in invoice_records:
            ...
            inv_id = each[9]
            nc_date = each[6].strftime("%Y-%m-%d %H:%M:%S")
...
def reverse_invoice_global_meli():
    ...
    try:
        ...
        for each in invoice_records:
            ...
            inv_id = each[9] # Almacena el ID de la factura
            inv_name = each[8] # Almacena el nombre de la factura
            #Busca la factura que contenga el nombre de la SO
...
def reverse_invoice_amazon():
    ...
    print('Vaya por un tecito o un café porque este proceso tomará algo de tiempo')
    #INDIVIDUALES AMAZON
    mycursor.execute("""# AMAZON TOTALES
                        #INDIVIDUALES
                        SELECT c.name,
                                   d.order_id 'order_id',
                                   c.channel_order_reference 'marketplace reference',
